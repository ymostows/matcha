...
export const PhotoUploadAdvanced: React.FC<PhotoUploadAdvancedProps> = ({ 
  ...
  useEffect(() => {
    const loadRealPhotos = async () => {
      try {
        console.log('🔄 PhotoUploadAdvanced: Chargement des photos...');
        const profile = await profileApi.getMyProfile();
        const realPhotos = profile.photos || [];
        console.log('📸 Photos chargées:', realPhotos.length, realPhotos.map(p => ({ id: p.id, filename: p.filename })));
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log('🎯 PhotoUploadAdvanced: Début upload de', files.length, 'fichier(s)');
    setIsUploading(true);
    setUploadError(null);

    try {
      const filesArray = Array.from(files);
      console.log('📤 Envoi vers profileApi.uploadPhotos...');
      ...
      console.error('❌ Erreur upload PhotoUploadAdvanced:', error);
