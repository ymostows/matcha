...
// Configuration pour la génération de profils avec photos
...
// Fonction pour générer un placeholder SVG personnalisé
function generatePlaceholderSVG(firstName: string, lastName: string, gender: 'homme' | 'femme'): string {
...
// Fonction pour créer une photo placeholder en base64
function createPlaceholderPhoto(firstName: string, lastName: string, gender: 'homme' | 'femme', photoIndex: number): string {
...
async function tryGetExternalPhoto(url: string): Promise<string | null> {
  try {
    ...
    console.warn(`API externe échouée (${url}):`, (error as Error).message);
    return null;
  }
}

// Fonction pour obtenir une photo avec fallback
async function getPhotoWithFallback(firstName: string, lastName: string, gender: 'homme' | 'femme', photoIndex: number): Promise<string> {
  // Liste des APIs à essayer
  const externalSources = [
    `https://picsum.photos/400/500?random=${Date.now()}-${photoIndex}`,
    `https://thispersondoesnotexist.com/image?t=${Date.now()}-${photoIndex}`,
...
async function createPhotosForUser(userId: number, firstName: string, lastName: string, gender: 'homme' | 'femme', photoCount: number = 3): Promise<void> {
...
async function generateProfilesWithPhotos(count: number = 10): Promise<void> {
  ...
  try {
    for (let i = 0; i < count; i++) {
      ...
      try {
        ...
        // Créer le profil
        const orientations: ('bi' | 'hetero' | 'homo')[] = ['bi', 'hetero', 'homo'];
...
export { generateProfilesWithPhotos };